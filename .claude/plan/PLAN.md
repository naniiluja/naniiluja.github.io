...
| 009 | Bật class-based dark mode (`@custom-variant`) + helper thuần `theme.ts` + inline FOUC script, mặc định tối (bỏ phụ thuộc OS); CHƯA có nút | css entry + helper + layout head | unit test `resolveTheme` + `nextTheme` xanh; `astro check` pass; `dist/index.html` có inline script toggle `.dark` trong `<head>`; preview localStorage trống → nền tối | 008 | done |
| 010 | Nút toggle text trong Nav (chuyển sáng↔tối, lưu localStorage), handler dùng helper thuần `nextTheme` | helper + component Nav | unit test `nextTheme` xanh; `dist/index.html` có `<button>` toggle + script set `localStorage.theme`; verify preview: bấm đổi + reload giữ lựa chọn + clear → về tối | 009 | done |
